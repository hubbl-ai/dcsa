/*
DCSA OpenAPI specification for Electronic Bill of Lading

API specification issued by DCSA.org.  For explanation to specific values or objects please refer to the <a href='https://dcsa.org/wp-content/uploads/2020/12/20201208-DCSA-P1-DCSA-Information-Model-v3.0-FINAL.pdf'>Information Model v3.0</a>  It is possible to use this API as a standalone API. In order to do so it is necessary to use the poll-endPoint - /v1/events  in order to poll event information.  It is recomended to implement the <a href='https://app.swaggerhub.com/apis/dcsaorg/DOCUMENTATION_EVENT_HUB'>DCSA Documentation Event Hub</a> in order to use the push model. Here events are pushed as they occur.

API version: 1.0.0
Contact: info@dcsa.org
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package bol

import (
	"encoding/json"
)

// DocumentParties struct for DocumentParties
type DocumentParties struct {
	DocumentParties []DocumentParty `json:"documentParties,omitempty"`
}

// NewDocumentParties instantiates a new DocumentParties object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDocumentParties() *DocumentParties {
	this := DocumentParties{}
	return &this
}

// NewDocumentPartiesWithDefaults instantiates a new DocumentParties object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDocumentPartiesWithDefaults() *DocumentParties {
	this := DocumentParties{}
	return &this
}

// GetDocumentParties returns the DocumentParties field value if set, zero value otherwise.
func (o *DocumentParties) GetDocumentParties() []DocumentParty {
	if o == nil || o.DocumentParties == nil {
		var ret []DocumentParty
		return ret
	}
	return o.DocumentParties
}

// GetDocumentPartiesOk returns a tuple with the DocumentParties field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DocumentParties) GetDocumentPartiesOk() ([]DocumentParty, bool) {
	if o == nil || o.DocumentParties == nil {
		return nil, false
	}
	return o.DocumentParties, true
}

// HasDocumentParties returns a boolean if a field has been set.
func (o *DocumentParties) HasDocumentParties() bool {
	if o != nil && o.DocumentParties != nil {
		return true
	}

	return false
}

// SetDocumentParties gets a reference to the given []DocumentParty and assigns it to the DocumentParties field.
func (o *DocumentParties) SetDocumentParties(v []DocumentParty) {
	o.DocumentParties = v
}

func (o DocumentParties) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.DocumentParties != nil {
		toSerialize["documentParties"] = o.DocumentParties
	}
	return json.Marshal(toSerialize)
}

type NullableDocumentParties struct {
	value *DocumentParties
	isSet bool
}

func (v NullableDocumentParties) Get() *DocumentParties {
	return v.value
}

func (v *NullableDocumentParties) Set(val *DocumentParties) {
	v.value = val
	v.isSet = true
}

func (v NullableDocumentParties) IsSet() bool {
	return v.isSet
}

func (v *NullableDocumentParties) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDocumentParties(val *DocumentParties) *NullableDocumentParties {
	return &NullableDocumentParties{value: val, isSet: true}
}

func (v NullableDocumentParties) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDocumentParties) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
